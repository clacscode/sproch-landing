...
import { ArrowRight, ArrowUpRight, CalendarDays, HeartPulse } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { NewsCard } from "@/components/public/NewsCard";
import { NewsSearch } from "@/components/public/NewsSearch";
import { formatDate } from "@/lib/format";
import { listAllPatientTags, listPatients } from "@/server/queries/patients";
...
export const metadata: Metadata = {
  title: "Pacientes",
  description:
    "Información y orientación para pacientes sobre prótesis y rehabilitación oral, de la Sociedad de Prótesis y Rehabilitación Oral de Chile.",
};
...
export default async function PatientsListingPage({
  searchParams,
}: {
  ...
  const [articles, tags] = await Promise.all([
    listPatients({ tag: params.tag, q: params.q }),
    listAllPatientTags(),
  ]);
        ...
        <div className="container-page relative pb-20 pt-28 md:pb-28 md:pt-36 lg:pb-32 lg:pt-44">
          <div className="grid items-end gap-12 md:grid-cols-12">
            ...
            <div className="md:col-span-4 md:pb-4">
              <p className="max-w-md text-base text-ink-200 md:text-lg">
                Orientación clara y confiable sobre prótesis y rehabilitación oral: qué tratamientos
                existen, cómo prepararte y cómo cuidar tu boca. Pensado para ti.
              </p>
            </div>
          </div>
          ...
          <div className="mt-16 flex flex-wrap items-center gap-x-3 gap-y-3 border-t border-white/10 pt-8 text-xs uppercase tracking-[0.22em] text-ink-300 md:mt-20 md:gap-x-6 md:text-sm">
            <span className="inline-flex items-center gap-2">
              <HeartPulse size={14} className="text-brand-600" aria-hidden />
      ...
      <section className="bg-paper">
        <div className="container-page py-12 md:py-16">
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            <NewsSearch basePath="/pacientes" initialQuery={params.q ?? ""} initialTag={params.tag} />
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
            <div className="-mx-4 flex flex-1 gap-2 overflow-x-auto px-4 pb-2 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0 sm:pb-0">
              <FilterPill href={params.q ? `/pacientes?q=${encodeURIComponent(params.q)}` : "/pacientes"} active={!params.tag}>
                Todos
              </FilterPill>
              {tags.map((tag) => {
          ...
          {articles.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-ink-200 bg-white px-6 py-16 text-center">
              ...
              <p className="mt-3 text-ink-600">
                {params.q
                  ? <>No encontramos artículos que coincidan con <strong>&ldquo;{params.q}&rdquo;</strong>{params.tag ? <> en la categoría <strong>{params.tag}</strong></> : null}.</>
            ...
            <>
              {(!params.tag && !params.q && rest.length > 0) || params.tag || params.q ? (
                <div className="mb-8 flex items-center justify-between border-b border-ink-200 pb-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.22em] text-ink-500">
                    {params.tag || params.q ? "Resultados" : "Más artículos"}
                  </p>
              ...
              <ul className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {(params.tag || params.q ? articles : rest).map((article) => (
                  <li key={article.id}>
                    <NewsCard article={article} basePath="/pacientes" readLabel="Leer artículo" />
                  </li>
                ))}
              </ul>
            </>
